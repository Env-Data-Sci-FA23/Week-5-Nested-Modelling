...
```{r}
daily_conc <- function(dataframe){
  conc_wide <- readWQPdata(siteid = dataframe$siteid,
                            sampleMedia = "Water",
  ...
  # Generate a variable name based on the input dataframe
  output_var_name <- paste("conc_daily_", deparse(substitute(dataframe)), sep="")
  
  # Assign the result to a variable with a unique identifier in the global environment
  assign(output_var_name, conc_daily, envir = .GlobalEnv)
  
  cat("The result has been saved to a variable named '", output_var_name, "'.\n", sep="")
  
  invisible(conc_daily)  # Return the result invisibly
}
...
```{r}
ad_colorado <- tibble(siteid = c('USGS-09180000', 'USGS-09180500', 'USGS-09380000'),
                          basin = c('dolores', 'colorado4', 'colorado5'))

daily_conc(ad_colorado)
```
...
# Combine the data from 'conc_daily' and 'conc_daily_ad_colorado'

join_conc <- conc_daily %>%
  full_join(conc_daily_ad_colorado)


join_conc %>% arrange(date)

saveRDS(join_conc, file = 'data/conc_tidied_full_wq.RDS')
```
...
sites <- colorado %>%
  ...
  bind_rows(ad_colorado) %>%
  #Grab just the column labeled sites
  pull(siteid) %>%
  #Remove the USGS- prefix
  gsub('USGS-', '', .)

Q_wide <- readNWISdv(siteNumbers = sites, 
                     parameterCd = "00060", 
...
Q_wide <- renameNWISColumns(
  Q_wide,
  X_00060_00003 = "Flow", 
  X_00060_00003_cd = "Flow_cd"
  
)

saveRDS(Q_wide, file = 'data/g_Q.RDS')
```
