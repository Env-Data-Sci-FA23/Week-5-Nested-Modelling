...
process_water_quality <- function(sites_tibble) {
  # Define the parameters for water quality
  ...
  conc_wide <- readWQPdata(siteid = sites_tibble$siteid,
                           sampleMedia = "Water",
  ...
  # Filter for mg/L units and convert date
conc_tidy <- conc_meta %>% 
  filter(units == 'mg/l') %>%
  mutate(date = ymd(date)) %>%
  select(date,
...
# Use the function to process this new data
additional_conc_daily <- process_water_quality(additional_data)

# You can view the resulting data
print(additional_conc_daily)
```
...
wq <- bind_rows(conc_daily, additional_conc_daily)
...
discharge_data <- lapply(sites, function(site) {
  readNWISdv(siteNumbers = site, parameterCd = '00060', startDate = "1980-10-01", endDate = "2023-11-01") %>%
    renameNWISColumns()
})


q_data <- bind_rows(discharge_data)
