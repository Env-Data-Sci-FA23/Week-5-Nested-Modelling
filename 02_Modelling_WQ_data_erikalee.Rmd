...
tidied_wq_2_models <- wq_2_models %>%
  mutate(mods_tidied = purrr::map(mods, tidy)) %>%
  select(-mods)
...
```{r}
#***NOT DONE WITH THIS... still working on first line of code
wq_2_mod_summaries <- tidied_wq_2_model_unnest %>%
  select(siteid, mods_tidied, p.value, slope) %>%
  mutate(trend = ifelse(p.value < 0.01, 'yes', 'no')) # create a column telling us whether or not there was a significant trend based on a p-value cut-off of 0.01
