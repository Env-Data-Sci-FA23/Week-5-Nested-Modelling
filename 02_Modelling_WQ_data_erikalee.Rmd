...
```{r}
#this tests the conc column of dolores_ca dataframe
...
```{r}
tidy_dolores <- tidy(dolores_slope) %>%
  #to add slope to tidy'd dataframe
  mutate(slope = dolores_slope$estimates)
```
...
low_nest <- low_flow %>%
  ...
  rename(characteristic = parameter) %>%# rename 'parameter'
  group_by(characteristic, basin) %>% #group by characteristic and basin
  nest() 
...
#.x$Y - y is the column within nested data column, output is more than one row so it has to be nested
#tidy_mods is a new column created

wq_models
```

```{r}
#to unnest the tidy_mods column but keep the rest of the dataframe the same... 
wq_models_unnest <- wq_models %>%
  unnest(tidy_mods)
```
...
Q_2 <- Q %>%
  ...
  select(-c(Date, site_no))

tidied_full_wq_2 <- inner_join(tidied_full_wq, Q_2, by = c("siteid", "date"))
```
...
dolores_chloride <- tidied_full_wq_2 %>%
  filter(siteid == "USGS-09180000", parameter == "Chloride")

ggplot(dolores_chloride, aes(x = conc, y = Flow)) + 
  geom_point() +
  scale_x_log10() +
  theme(legend.pos = c(0.7, 0.2),
        legend.direction = 'horizontal') +
  ylab('Flow (cfs)') +
  xlab('Ion Concentration')
```

#### What do you see in this relationship?

Answer: The concentration increases as the flow, or discharge, decreases.
...
```{r}
wq_2_nested <- tidied_full_wq_2 %>%
  group_by(parameter, basin) %>%
  nest()
```
...
```{r}
#not sure if this gave me the correct model, because I have an intercept and flow list for each variable?
wq_2_models <- wq_2_nested %>%
...
tidied_wq_2_models <- wq_2_models %>%
  mutate(mods = map(mods, ~tidy(.x)))

#unnest mods_tidied column
wq_2_model_unnest <- tidied_wq_2_models %>%
  unnest(mods)
```
...
```{r}
wq_2_mod_summaries <- wq_2_model_unnest %>%
  filter(term == "Flow") %>%
  mutate(trend = ifelse(p.value < 0.01, 'yes', 'no'))

ggplot(wq_2_mod_summaries, aes(x = parameter, y = estimate, color = trend)) + 
  geom_point() + 
  facet_wrap(~basin, scales = 'free') + 
  theme_minimal() + 
  scale_color_manual(values = c('black','green3')) + 
...
The **`furrr::map`** function is similar to **`purrr::map`**, but it allows you to perform parallel mapping using the future package. Futures is a way of expressing parallel and distributed computations in R.

You'd want to use `furrr` when you have a large number of independent tasks that can be paralleled. It is especially helpful for tasks that involve a lot of computation.
