...
# inner join q and wq data frames
q_and_wq <- inner_join(q, wq, by = c("siteid", "date"))
...
# dolores basin & calcium concentration 
q_and_wq %>%
  filter(basin == "dolores" & parameter == "Calcium") %>%
  ggplot(aes(x = Flow, y = conc)) +
  geom_point() +
  labs(title = "Scatter Plot of Calcium Concentration vs. Discharge",
       x = "Discharge", y = "Calcium Concentration")

```

#### What do you see in this relationship?
The greater the discharge the lower the calcium concentration is in the dolores basin. 
...
q_and_wq_model <- q_and_wq_nested %>%
...
```{r}
q_and_wq_tidy <- q_and_wq_model %>%
  mutate(mods = map(mods, ~tidy(.x)))

q_and_wq_unnest <- q_and_wq_tidy %>%
  unnest(mods)
```
...
```{r}
model_summary <- q_and_wq_models %>%
  filter(term == "Flow")%>%
  mutate(trend = ifelse(p.value < 0.01, 'yes','no'))

ggplot(model_summary,aes(x = characteristic, y = slope, color = trend))+
  geom_point()+
  facet_wrap(~basin, scales = 'free') + 
  theme_minimal() + 
  scale_color_manual(values = c('black','green3')) + 
