...
```{r}
q_discharge <- readRDS('data/Q.RDS')

q_discharge <- q_discharge %>%
  mutate(site_id =paste0("USGS-", q_discharge$site_no))

# join discharge to water quality data by date and siteid:
# using an inner join

wq_discharge <- tidied_full_wq %>%
  inner_join(q_discharge, by = c('siteid'='site_id', 
             'date'='Date'))
```
...
```{r}
# I am choosing site USGS-09180500 and Magnesium
# need to filter by site and ion
wq_dis_magnesium <- wq_discharge %>%
  filter(siteid == "USGS-09180500", parameter == "Magnesium")

ggplot(wq_dis_magnesium, aes(x = Flow, y = conc)) +
  geom_point() +
  theme_minimal() +
...
This looks like a curvilinear relationship.
...
```{r}
wq_discharge_nest <- wq_discharge %>%
...
wq_discharge_nest <- wq_discharge_nest %>%
  mutate(linmod = map(data, ~lm(conc ~ Flow, data = .x)))
...
wq_discharge_nest <- wq_discharge_nest %>%
  mutate(tidy_linmod = map(linmod, ~tidy(.x)))
...
wq_discharge_unnest <- wq_discharge_nest %>%
  unnest(tidy_linmod) %>%
  select(basin, parameter, estimate, p.value, term) %>%
  mutate(trend = ifelse(p.value < 0.01, "yes", "no"))

# filtering term column so we only have slope and not intercept
wq_discharge_unnest_filtered <- wq_discharge_unnest %>%
  filter(term == "Flow")

summary(wq_discharge_unnest)

# plotting the model relationship:
ggplot(wq_discharge_unnest_filtered, aes(x = parameter, y = estimate, color = trend)) + 
  geom_point() + 
  facet_wrap(~basin, scales = 'free') + 
  theme_bw() + 
...
The furrr package is a combination of the future and purrr package and it runs code in parallel rather than sequentially, like purrr does, which allows it to execute its computations faster.

When would you want to use this `furrr` function?

I think furrr could be useful when mapping/computing across a very large dataframe that doesn't need to be ran sequentially. I assume this means that the function that you are applying using furrr wouldn't need make a computation based on a previous row value, for example. Like if you were using the cumulative sum function to populate a new column based on the count in an existing column, you would want to use purrr instead of furrr (?).
