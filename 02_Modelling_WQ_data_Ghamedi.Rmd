...
wq <- readRDS('data/conc_tidied_full_wq.RDS')
...
Q$siteid <- paste0(Q$agency_cd, "-", Q$site_no)

# Get rid of the original agency_cd and site_no

Q <- Q[, !(names(Q) %in% c("agency_cd", "site_no"))]

# Rename the new column
colnames(Q)[colnames(Q) == "siteid"] <- "siteid"

colnames(Q)[colnames(Q) == "Date"] <- "date"

complete_conc <- inner_join(tidied_full_wq, Q, by = "date", "siteid")
...
colorado4_ca <- complete_conc %>%
  filter(basin == 'colorado4', parameter == 'Magnesium') 

ggplot(colorado4_ca, aes(x = conc, y = Flow)) + 
  geom_point()
...
The higher the flow, the lower the concentration of Magnesium
...
complete_conc <- complete_conc[, !(names(complete_conc) %in% c("siteid.y"))]

colnames(complete_conc)[colnames(complete_conc) == "siteid.x"] <- "siteid"

high_flow <- complete_conc %>%
  mutate(month = month(date),
         year = year(date)) %>% # create columns of just month and year
  filter(month %in% c(8,9,10,11)) %>% #filter later summer months
  group_by(basin, siteid, parameter, year) %>%
...
ggplot(high_flow, aes(x = year, y = conc, color = basin)) + 
  facet_wrap(~parameter, scales = 'free') + 
  geom_point() + 
  theme_minimal() + 
  scale_y_log10() + 
