...
daily_conc_fun<- function(input_tibble, parameters, startDateLo, startDateHi) {

    conc_wide <- readWQPdata(siteid = input_tibble$siteid,
                           sampleMedia = "Water",
    ...
    mutate(units = trimws(units))
       ...
       conc_meta <- conc_small %>%
    left_join(., input_tibble, by = "siteid") %>%
...
conc_more_data <- daily_conc_fun(additional_data, parameters, startDateLo = "1980-10-01", startDateHi = "2023-11-01" )
```
...
wq <- bind_rows(conc_daily, conc_more_data)
