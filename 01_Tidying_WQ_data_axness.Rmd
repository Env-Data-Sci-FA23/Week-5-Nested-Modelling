...
process_water_quality_data <- function(site_data) {
  # Define the parameters for water quality data
  parameters <- c("Calcium", "Magnesium", "Sodium", "Potassium", 
                  "Sulfate", "Sulfate as SO4", "Sulfur Sulfate", 
                  "Total Sulfate", "Chloride", "Alkalinity, bicarbonate", "Bicarbonate")
  ...
  conc_small <- conc_wide %>%
    select(date = ActivityStartDate,
           ...
           siteid = MonitoringLocationIdentifier,
           value = ResultMeasureValue) %>%
    mutate(units = tolower(as.character(units)))  # Convert units to lowercase

  # Add basin information, ion aliases, and rename 'value' to 'conc'
  conc_meta <- conc_small %>%
    ...
    mutate(ion = case_when(
      ...
      parameter %in% c("Alkalinity, bicarbonate", "Bicarbonate") ~ "HCO3",
      TRUE ~ NA_character_),
    conc = as.numeric(value))  # Convert value to numeric and rename to conc

  # Filter to keep only mg/L units
  conc_tidy <- conc_meta %>% 
    filter(units == 'mg/l') %>%
    mutate(date = ymd(date)) %>%
    select(date, ion, siteid, basin, conc)

  # Calculate daily averages
  conc_daily <- conc_tidy %>%
    group_by(date, ion, siteid, basin) %>% 
...
# colorado <- tibble(siteid = c("USGS-09034500", "USGS-09069000", ...),
#                   basin = c("colorado1", "eagle", ...))
# new_daily_data <- process_water_quality_data(colorado)
...
```{r}
# new site IDs and basins
additional_data <- tibble(siteid = c('USGS-09180000', 'USGS-09180500', 'USGS-09380000'),
                          basin = c('dolores', 'colorado4', 'colorado5'))

# function from Question 1 to process the new data
additional_data_cleaned <- process_water_quality_data(additional_data)

# view the cleaned data
View(additional_data_cleaned)
...
# Create a mapping from parameter to ion
parameter_to_ion <- c("Calcium" = "Ca", "Magnesium" = "Mg", "Sodium" = "Na", 
                      "Potassium" = "K", "Sulfate" = "SO4", "Chloride" = "Cl", "Bicarbonate" = "HCO3"
                      )

# Add ion column to conc_daily based on parameter column
conc_daily <- conc_daily %>%
  mutate(ion = parameter_to_ion[parameter])

# Combine conc_daily and additional_data_cleaned
combined_data <- bind_rows(conc_daily, additional_data_cleaned)

# Save the combined data as RDS
dir.create("data", showWarnings = FALSE)  # Create the directory if it doesn't exist, suppress warnings if it does
saveRDS(combined_data, file = 'data/tidied_full_wq_axness.RDS')
...
# Download daily discharge data for all sites
...
dir.create("data", showWarnings = FALSE)  # Create the directory if it doesn't exist, suppress warnings if it does
saveRDS(q_data, file = 'data/Q_axness.RDS')
