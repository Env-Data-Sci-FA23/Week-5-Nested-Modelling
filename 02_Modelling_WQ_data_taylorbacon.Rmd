...
```{r}
#Plot sodium concentration vs discharge for colorado1
q_and_wq %>%
  filter(basin == "colorado1" & parameter == "Sodium")%>%
  ggplot()+
  geom_point(aes(x=Flow,y=conc))
```

#### What do you see in this relationship?

This relationship looks like there is an exponential decay trend.
...
#<<<< YOUR CODE HERE >>>>
mod_summaries <- q_wq_models %>%
  unnest(tidy_mods)%>%
  filter(term == "Flow")%>%
  select(basin, characteristic, p.value, estimate)%>%
