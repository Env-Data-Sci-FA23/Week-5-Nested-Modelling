...
Q_U<-mutate(Q,siteid=paste0("USGS-",site_no))%>%
  rename(date=Date)


Q_tidy<-inner_join(Q_U,tidied_full_wq, by=c("date","siteid"))
...
#<<<< YOUR CODE HERE >>>>#
Q_filtered<- filter(Q_tidy, siteid=="USGS-09380000", parameter=="Chloride" )

ggplot(Q_filtered)+
geom_line(aes(x=Flow,y=conc),color="green")
...
#<<<< YOUR CODE HERE >>>>#
lm_Q_fill<-Q_fill%>%
mutate(linear_mods = map(data, ~lm(conc ~ Flow, data =.x)))
```
...
Q_mod_summaries <- lm_Q_fill %>%
  (tidy=lm_Q_fill$linear_mods)
