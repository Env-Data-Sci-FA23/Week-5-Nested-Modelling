...
```{r setup, warnings=FALSE, message=FALSE, echoe = TRUE, tidy=TRUE}
source('setup.R')
```
...
# Making Q data look exactly like wq data
q_data_wq_same <- q_data %>%
  mutate(siteid = paste0('USGS-',site_no)) %>%
  rename(date = Date)

q_wq <- inner_join(q_data_wq_same,wq, by = c('date','siteid'))


wq_q_same <- wq %>%
  mutate(site_no = gsub('USGS-','',siteid)) %>%
  rename(Date = date)

wq_q <- inner_join(wq_q_same, q_data, by = c('Date','site_no'))
```
...
```{r}
wq_q %>%
  filter(siteid == "USGS-09380000" & parameter == "Chloride") %>%
  ggplot() +
  geom_point(aes(x = Flow, y = conc), color = "black") +
  theme_minimal()
```
...
```{r}
wq_q_nest <- wq_q %>%
  # get the log of the CFS... Flow is better as log almost always
...
```{r}
my_mod <- function(df = wq_q_nest$data[[1]]){
  mod <- lm(conc ~ log_flow, data = df)

}
wq_q_mods <- wq_q_nest %>%
  # only need to do do one of these jabroneys
  mutate(mods = map(data, ~lm(conc ~ log_flow, data = .x)),
         mod2 = map(data, my_mod))
```
...
```{r}
tidier_lm <- function(wq_mod = wq_q_mods$mods[[1]]){
  tidier_mod <- tidy(wq_mod) %>%
    mutate(r_squared = summary(wq_mod)$adj.r.squared)
}

wq_tidy_mods <- wq_q_mods %>%
  mutate(tidy_output = map(mods, tidier_lm)) %>%
  select(-data, -mod2) %>%
  unnest(tidy_output)
```
...
```{r}
for_plotting <- wq_tidy_mods %>% 
  filter(term == "log_flow") %>% 
  mutate(trend = if_else(p.value <= 0.05, "significant", "non-significant"))

ggplot(for_plotting, aes(x = parameter, y = estimate, color = trend)) + 
  geom_point() + 
  facet_wrap(~basin, scales = 'free') + 
  theme_minimal() + 
  scale_color_manual(values = c('black','green3')) + 
