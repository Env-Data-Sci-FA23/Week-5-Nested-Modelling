...
wq <- wq %>%
mutate(site_no = str_remove(siteid, "USGS-")) %>%
  rename(Date = "date")

Q_WQ_joined <- inner_join(Q, wq, by = c("Date", "site_no"))
...
roaring_sodium <- Q_WQ_joined %>%
  filter(basin == 'roaring', parameter == 'Sodium') 

ggplot(roaring_sodium, aes(x = Flow, y = conc)) + 
  geom_point()+
...
There appears to be a strong correlation as the plot looks like a negative curve. I added a loess curve to show the line of correlation.
...
Q_WQ_models <- Q_WQ_nested %>%
  mutate(mods = map(data, ~summary(lm(conc ~ Flow, data = .x)))) 

summary(Q_WQ_models$mods[[1]])
...
Q_WQ_mods_tidy <- Q_WQ_models %>%
  mutate(mods = map(mods, ~tidy(.x)))

Q_WQ_tidy_unested <- Q_WQ_mods_tidy %>%
  unnest(mods) 
```
...
filtered_Q_WQ <- Q_WQ_tidy_unested %>%
  filter(!grepl("Intercept", term)) %>% 
  mutate(trend = ifelse(p.value < 0.01, 'yes', 'no'))
  


ggplot(filtered_Q_WQ,aes(x = characteristic, y = estimate, color = trend)) + 
  geom_point() + 
  facet_wrap(~basin, scales = 'free') + 
  theme_minimal() + 
  scale_color_manual(values = c('black','green3')) + 
