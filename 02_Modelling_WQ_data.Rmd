...
dolores_ca <- dolores_ca %>%
  #Make sure data is arranged from 1980 onward using `arrange()` 
  arrange(date)
...
Ok, now we have an elaborate way to confirm what the plot already showed us. There is no long-term trend in Calcium concentrations in the Dolores river (denoted by a high p-value of 0.291, much greater than our usual 0.05 alpha/cut-off).
...
low_flow <- wq %>%
  ...
  filter(month %in% c(8,9,10,11)) %>% #fitler later summer months
...
For example, to retrieve one of those nested dataframes:

```{r}
low_nest$data[[1]] #index the tibble in the first row of 'data', using double brackets [[]] since we are indexing a list
```

### Modelling over nested data

Now we want to apply our model to each nested dataframe. To do this we need to use the `map()` function. Map takes in an x (`data` column) and then a function (in this case `sens.slope`). We use `.x$conc` to indicate that we want to apply the model to the concentration column within each bundled (nested) data frame.

```{r}
wq_models <- low_nest %>%
  mutate(mods = map(data, ~ sens.slope(.x$conc))) # create a new column to store the model results of each row/dataset
...
wq_mod_summaries <- wq_models %>%
  mutate(tidy_mods = map(mods, tidier)) %>% # create a new column to store the results of tidier()
