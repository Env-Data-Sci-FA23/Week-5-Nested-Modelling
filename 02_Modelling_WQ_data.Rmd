...
Use `inner_join` to join our daily discharge data (`Q.RDS`) to our raw water quality data (`tidied_full_wq.RDS`). You want to join by both date and siteid. Remember! the discharge data has site IDs that we had to drop the `USGS-` from, so you will need to add that back in using `paste0`. (1 point)
...
### Pick any site and ion combination and plot discharge versus ion concentration (1 point)
...
Group your data by basin and water quality parameter and nest the data. (2 point)
...
You will need to use a `map` command like this: `map(data, ~lm(conc ~ q, data = .x))` (2 point)
...
You should have a new column called `mods` or something similar, and you need to `tidy` those mods and store this new, tidier data in another column. (2 point)
...
## Make a visual of your models' summaries that shows both which sites have significant relationships between discharge and concentration and the slope of that relationship. (2 point)
...
Look up the `furrr` package. What does `furrr::map` do that is different from `purrr::map`? (2 extra points)
