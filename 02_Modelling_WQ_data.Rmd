...
Q$site_no <- paste0(usgs, "-", Q$site_no)

colnames(Q)[colnames(Q) == "Date"] <- "date"
colnames(Q)[colnames(Q) == "site_no"] <- "siteid"

stream_data <- inner_join(wq, Q, by = c("date", "siteid"))
...
#<<<< YOUR CODE HERE >>>>#
roaring_sulf <- stream_data %>%
  filter(c(basin == "roaring"), (parameter == "Sulfate"))

ggplot(roaring_sulf, aes(x = Flow, y = conc)) +
  geom_point()
...
As flow increases, Sulfate ion concentration decreases. 
...
stream_mod_tidy2 <- stream_model %>%
  mutate(tidy = map(data, ~ tidier_sens(.x$conc)))
...
stream_mod_sum <- stream_mod_tidy2 %>%
  unnest(tidy) %>%
  select(basin, characteristic, p.value, slope) %>%
  mutate(trend = ifelse(p.value < 0.01, 'yes', 'no'))

ggplot(stream_mod_sum, aes(x = characteristic, y = slope, color = trend)) +
  geom_point() +
  facet_wrap(~basin, scales = 'free') +
  theme_minimal() +
  scale_color_manual(values = c('black', 'green3')) +
