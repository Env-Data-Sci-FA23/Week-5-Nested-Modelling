...
water_data <- function(x) { 
  ...
  conc_wide <- readWQPdata(siteid = new_data$siteid,
                         sampleMedia = "Water",
...
```{r}
new_water_data <- water_data(additional_data)
```
...
```{r}
tidied_full_wq <- full_join(conc_daily, new_water_data)
```
