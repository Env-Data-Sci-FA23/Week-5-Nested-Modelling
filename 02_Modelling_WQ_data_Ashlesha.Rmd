...
# Joining Q and WQ data
q_data <- readRDS('data/Q.RDS')%>%
  mutate(site_no = paste0("USGS-",site_no))%>%
  rename("siteid" = site_no,
         "date" = Date)

combined_data <- inner_join(q_data,wq, by = c("siteid", "date"))
...
# Filter data for a specific site and ion
site_ion_data <- combined_data %>%
  filter(siteid == 'USGS-09180000', parameter == 'Calcium')

# Plot discharge vs ion concentration
ggplot(site_ion_data, aes(x = Flow, y = conc)) +
  geom_point() +
  labs(title = "Discharge vs Calcium Concentration",
       x = "Discharge (cfs)",
...
Thois graph shows the relationship between stream discharge (measured in cubic feet per second, or cfs) and calcium concentration (measured in milligrams per liter, mg/L). There is a negative correlation between discharge and calcium concentration. It seems like the graph is inversely proportional. (y= k/x, for constant k) 
...
nested_data <- nested_data %>%
  mutate(lm_mods = map(data, ~ lm(conc ~ Flow, data = .x)))
...
model_summaries <- nested_data %>%
  mutate(tidy_mods = map(lm_mods, ~tidy(.x)))
...
model_summaries1 <- model_summaries %>%
  unnest(tidy_mods)%>%
  filter(term == "Flow")%>%
  select(basin, parameter,estimate, p.value, term)%>%
  mutate(trend = ifelse(p.value < 0.01, 'yes','no'))

```

```{r}
ggplot(model_summaries1, aes(x = parameter, y = estimate, color = trend)) + 
  geom_point() + 
  facet_wrap(~basin, scales = 'free') + 
  theme_bw() + 
...
furrr::map is a function from the furrr package in R, it is an extension of the purrr package. The main difference between furrr::map and purrr::map is in how they handle tasks. furrr::map can do several tasks at the same time using parallel processing, which is great for big jobs or when you have a powerful computer. On the other hand, purrr::map does one task at a time, in order, which can be simpler but might take longer for large tasks.


I would use furrr::map over purrr::map when I have a task that is both time-consuming and can be broken down into independent subtasks that don't rely on the results of each other. This is typically the case in scenarios such as large-scale simulations, complex data processing, or applying a function to a large list where each function call is independent of the others.
