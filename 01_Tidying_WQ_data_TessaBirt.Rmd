...
```{r}
object1 <- tibble(siteid = c("x"),
                  basin = c("y"))


process_site_data <- function(object1) {
  conc_wide <- readWQPdata(siteid = object1$siteid, # pulling our siteids column in from the colorado object, becomes a vector
                         sampleMedia = "Water",
...
new_water_data <- process_site_data(additional_data)
```
...
tidied_full_wq <- rbind(conc_daily, new_water_data)
