...
```{r}
processSiteData <- function(x) {
  tibble <-x
parameters <- c("Calcium", "Magnesium", "Sodium", "Potassium", "Sulfate", "Sulfate as SO4", "Sulfur Sulfate", "Total Sulfate", "Chloride", "Alkalinity, bicarbonate", "Bicarbonate")
conc_wide <- readWQPdata(siteid = tibble$siteid, 
                         sampleMedia = "Water", 
...
```{r}
additional_water_data <- processSiteData(additional_data)
```
...
```{r}
combined_data <- bind_rows(conc_daily,additional_water_data)
...
site_numbers <- c('09180000', '09180500', '09380000')

Q <- readNWISdv(siteNumbers = sites,
               parameterCd = "00060",
