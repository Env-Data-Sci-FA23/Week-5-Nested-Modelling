...
```{r}
question1 <- tibble(sideid = c("x"), basin = c("y"))

question11 <- function(question1){

conc_wide <- readWQPdata(siteid = question1$siteid,
                         sampleMedia = "Water",
...
```{r}
question22 <- question11(additional_data)
```
...
```{r}
wq <- rbind(question22, conc_daily)
...
interested in. Save the data as an RDS object called `Q` in the data
folder: `data/Q.RDS`. The site numbers are the same as above but you
need to remove `USGS-` from each site. Discharge is
