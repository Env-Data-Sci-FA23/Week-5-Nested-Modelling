...
ggplot() + 
  geom_point(data = dolores_ca, aes(x = date, y = conc))
```
...
wq<-wq%>%
  ...
  separate(siteid, into = c("siteID", 'USGS'), sep = '-')
```

```{r}
wq<-wq%>%
  rename(ID = siteID, siteid = USGS)
```

Make the dataframes compatible, then inner_join

```{r}
Qwq <- inner_join(wq,Q, by = c("siteid", 'date'))
```

### Pick any site and ion combination and plot discharge versus ion concentration

```{r}
roaring_Bicarbonate <- Qwq%>%
  filter(basin == 'roaring', parameter == 'Bicarbonate')
```

```{r}
roaring_Bicarbonate%>%
  ggplot(aes(x=Flow, y=conc))+
  geom_point()+
  geom_smooth()
```

#### What do you see in this relationship? - as the flow increases the concentration decreases in a non-linear way.
...
```{r}
tidier_Qwq <- function(data){
  data <- as.data.frame(data)
  Qwq_model <- lm(conc ~ Flow, data = data)
  tidy_results <- tidy(Qwq_model)
}
```

```{r}
unnested_results <- nested_grouped_Qwq%>%
  unnest(mods)%>%
  unnest(data)
```
...
```{r}
nested_grouped_Qwq <- grouped_Qwq%>%
  mutate(mods = map(data, tidier_Qwq))
```
...
visual%>%
  ggplot(aes(x = characteristic, y = slope, color = trend))+
  geom_point() + 
  facet_wrap(~basin, scales = 'free') + 
  theme_minimal() + 
  scale_color_manual(values = c('black','green3')) + 
...
purrr runs sequentially, furrr runs in parallel decreasing computation time.

When would you want to use this `furrr` function?

when you have a large data set or are working with "computationally intensive" tasks.
