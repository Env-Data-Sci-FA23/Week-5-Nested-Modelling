...
```{r}
water_data <- function(x) {
tibble <- x
  
    parameters <- c("Calcium", "Magnesium", "Sodium", "Potassium", "Sulfate", "Sulfate as SO4", "Sulfur Sulfate", "Total Sulfate", "Chloride", "Alkalinity, bicarbonate", "Bicarbonate") 
  
    conc_wide2 <- readWQPdata(siteid = tibble$siteid, 
                                         sampleMedia = "Water",
                                         startDateLo = "1980-10-01", 
                                         startDateHi = "2023-11-01",
                                         characteristicName = parameters)
    conc_small2 <-  conc_wide2 %>%
  select(date = ActivityStartDate,
         parameter = CharacteristicName,
         units = ResultMeasure.MeasureUnitCode,
         siteid = MonitoringLocationIdentifier,
      ...
      conc_meta2 <- conc_small2 %>%
  left_join(.,tibble, by = "siteid") %>%
  dplyr::mutate(ion = dplyr::case_when(parameter == "Calcium" ~ "Ca",
                                       parameter == "Magnesium" ~ "Mg",
                                       parameter == "Sodium" ~ "Na",
                                       parameter == "Potassium" ~ "K",
...
```{r}
conc_daily2 <- water_data(additional_data)
```
...
tidied_full_wq <- full_join(conc_daily, conc_daily2)
...
q_data <- readNWISdv(sites, parameterCd = "00060") %>% 
  renameNWISColumns() %>% 
