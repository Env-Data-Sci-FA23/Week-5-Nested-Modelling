...
USGSQ<- mutate(Q,siteid = paste0("USGS-",site_no))%>%
  rename(date=Date)

tidyQ<- inner_join(USGSQ,tidied_full_wq,by = c("date","siteid"))
...
CO5SO4 <- tidyQ %>%
  filter(basin == 'colorado5', parameter == 'Sulfate') 

ggplot(CO5SO4, aes(x = Flow, y = conc)) + 
  geom_point()
