...
```{r}
# copy all data in evernote and work from the beginning to create this code chunk

other_conc_data <- function(site_IDs){
  
parameters <- c("Calcium", "Magnesium", "Sodium", "Potassium", "Sulfate", "Sulfate as SO4", "Sulfur Sulfate", "Total Sulfate", "Chloride", "Alkalinity, bicarbonate", "Bicarbonate")
  
conc_wide <- readWQPdata(siteid = site_IDs$siteid, 
                         sampleMedia = "Water",
...
```{r}
other_data <- tibble(siteid = c('USGS-09180000', 'USGS-09180500', 'USGS-09380000'),
                          basin = c('dolores', 'colorado4', 'colorado5'))

additional_conc_data <- other_conc_data(other_data)
```
...
tidied_full_wq <- rbind(conc_daily, additional_conc_data)

save(tidied_full_wq, file = 'data/tidied_full_wq.RDS')
```
...
sites <- colorado %>%
  ...
  bind_rows(additional_conc_data) %>%
