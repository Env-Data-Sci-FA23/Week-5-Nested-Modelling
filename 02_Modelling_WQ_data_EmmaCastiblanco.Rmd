...
merged_data_q_wq <- inner_join(wq, q_data, by=c ("siteid", "date") )
...
```{r}
site_ion_data<- merged_data_q_wq %>%
  filter(siteid=="USGS-09085000",parameter== "Chloride") %>%
  rename(q=X_00060_00003)

ggplot(site_ion_data,aes(x = q, y = conc)) + 
  geom_point() 

```

#### What do you see in this relationship? --\> as discharge increases, ion concentration decreases. 
...
```{r}
merged_data_q_wq<-merged_data_q_wq %>%
  rename(q=X_00060_00003)

nested_basin_water_quality<- merged_data_q_wq%>%
...
```{r}
linear_model<- nested_basin_water_quality %>%
mutate(mods= map(data, ~lm(conc ~ q, data = .x)))
...
```{r}
tidy_linear_model <- linear_model %>%
    mutate(tidy_results = map(mods, tidy))%>%
    select(tidy_results) %>%
  unnest(tidy_results)
```
...
```{r}
linear_summaries <- tidy_linear_model %>%
  select(basin, parameter, p.value, statistic) %>%
  mutate(trend = ifelse(p.value < 0.01, 'yes', 'no')) # create a column telling us whether or not there was a significant trend based on a p-value cut-off of 0.01
...
purrr functions run one after another, while furrr runs many simultaneously
