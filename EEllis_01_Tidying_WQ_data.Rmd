...
```{r}
table_gen <- function(sites){
  
  parameters <- c("Calcium", "Magnesium", "Sodium", "Potassium", "Sulfate", "Sulfate as SO4", "Sulfur Sulfate", "Total Sulfate", "Chloride", "Alkalinity, bicarbonate", "Bicarbonate")
  
  conc_daily <- readWQPdata(siteid = sites$siteid, # pulling our siteids column in from the colorado object, becomes a vector
                         sampleMedia = "Water", # WQP also has bilogical data and sediment data
                         startDateLo = "1980-10-01", # must be formatted as YYYY-MM-DD
                         startDateHi = "2023-11-01", # must be formatted as YYYY-MM-DD
                         characteristicName = parameters) %>% select(date = ActivityStartDate,
         parameter = CharacteristicName,
         units = ResultMeasure.MeasureUnitCode,
         siteid = MonitoringLocationIdentifier,
         org = OrganizationFormalName,
...
new_sites <- table_gen(additional_data)
new_sites
```
...
```{r}
full_data <- rbind(conc_daily, new_sites)

saveRDS(full_data, file = 'data/tidied_full_wq.RDS')
```
...
discharge_data <- readNWISdv(sites_dis, "00060", "1980-10-01", "2023-11-01", statCd = "00003")

discharge_data <- renameNWISColumns(discharge_data)

saveRDS(discharge_data, file = 'data/Q.RDS')
```
