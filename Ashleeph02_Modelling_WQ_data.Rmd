...
```{r}
discharge <- readRDS('data/Q.RDS')
discharge1 <- discharge %>%
  rename(date=Date) %>%
mutate(siteid = paste0('USGS-',site_no))
merged_data <- inner_join(wq, discharge1, by = c("date", "siteid"))
```
...
selected_ion <- "Calcium"

selected_data <- merged_data %>%
  filter(siteid == selected_site, parameter == selected_ion)
```

```{r}
# Create a scatter plot with switched axes
ggplot(selected_data, aes(x = conc, y = Flow)) +
  geom_point() +
  labs(title = paste("Ion Concentration vs Discharge at", selected_site),
       x = paste(selected_ion, "Concentration (mg/l)"),
       y = "Discharge (cms)") +
   stat_smooth(method = 'lm')
```

#### What do you see in this relationship?
The higher the Flow/Discharge amount, the lower the calcium concentration.
...
```{r}
tidier_lm1 <- function(data) {
  data <- as.data.frame(data)
  
  lm_models <- lm(conc ~ Flow, data = data)
  tidy_results <- tidy(lm_models)
  return(list(mods = list(lm_models), tidy_results = tidy_results))
}
```

```{r}
nest_low_merged_data <- low_merged_data %>%
  group_by(characteristic = parameter, basin) %>% 
  nest()
```

```{r}
nest_wq_models <- nest_low_merged_data %>%
  mutate(model_results = map(data, tidier_lm1))
```

```{r}
unnested_results <- nest_wq_models %>%
...
```{r}
nest_wq_models1 <- nest_low_merged_data %>%
  mutate(tidy_mods = map(data, tidier_lm))
...
```{r}
ggplot(nwq_mod_summary,aes(x = characteristic, y = slope, color = trend)) + 
  geom_point() + 
  facet_wrap(~basin, scales = 'free') + 
  theme_minimal() + 
  scale_color_manual(values = c('black','green3')) + 
...
The goal of furrr is to combine purrr’s family of mapping functions with future’s parallel processing capabilities.The furrr::map function is similar to the purrr::map function, but it automatically parallelizes the mapping process across multiple cores.

When would you want to use this `furrr` function?
You would want to use the furrr::map function to take advantage of parallel processing, potentially reducing the total execution time compared to the sequential mapping with purrr::map.
