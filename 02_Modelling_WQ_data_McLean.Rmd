...
```{r}
combined_data <- inner_join(Q, wq, by = c("date", "siteid"))
```
...
```{r}
#<<<< YOUR CODE HERE >>>>#
plotdata1 <- combined_data %>% 
  filter(basin == "roaring") %>% 
  filter(parameter == "Sodium")
```


```{r}
ggplot(data = plotdata1, mapping = aes(x = Flow, y = conc))+
  geom_point()
...
It looks like concentration drops as flow increases
...
#<<<< YOUR CODE HERE >>>>#
Qwq_models_summaries <- Qwq_models1 %>%
  mutate(mods = map(mods, ~tidy(.x))) %>% 
  unnest(mods)
 
  
  Qwq_models_summaries
...
Filtered_slopes <- Qwq_models_summaries %>% 
  filter(term == "Flow") %>% 
...
```{r}
ggplot(Filtered_slopes, aes(x = characteristic, y = estimate, color = trend)) + 
  geom_point() + 
  facet_wrap(~basin, scales = 'free') + 
  theme_minimal() + 
  scale_color_manual(values = c('black','green3')) + 
...
It runs your code in parallel rather than sequentially

When would you want to use this `furrr` function?

You would want to use this if you have a large amount of processing to do over multiple data points. Using purr they could be done in parallel and would take less time
