...
Let's first load in all of our necessary packages. Some of these
packages might be new to you, so be sure to install ahead of trying to
load them in!

```{r setup, warnings = 'hide', message = FALSE}
library(tidyverse) # dplyr, tibble, readr, and others to help clean coding
library(dataRetrieval) # downloads data from the Water Quality Portal and the USGS
...
now run `getWQP()` function

```{r}
# parameters %>%
#   map_dfr(~readWQPdata(siteid = colorado$siteid, # pulling our siteids column in from the colorado object, becomes a vector
...
#             characteristicName = .x)) # our vector of `characteristcName`s
...
Now let's look at the beginning of this tidier version:
...
methods used by the USGS are reasonable and exchangeable (one method is
equivalent to the other). If we make that assumption then the only
remaining data we need to clean is to make sure that all the data has
the same units.
...
it, actually all of it). That makes our job really easy.
...
columns, while adding a time object column using the `lubridate::ymd`
call.
...
current `conc_clean` data set and 2) how to write a function to
download, clean, and update our data with far less code.

## Question 1

Write a function that can repeat the above steps with a single function
call. This function should take in a single tibble that is identical in
structure to the `colorado` one above (e.g. it has columns named
`siteid` and `basin`). The function should then take in that tibble and
be able to download and clean data for any of the site IDs below to make
the data look exactly like `conc_daily`.
...
our original `conc_daily` data set. The steps are:

1.  *In the function* - Download the new data

2.  *In the function* - Clean the new data and make its structure look
    identical to `conc_tidy`

3.  *With the new data the function returns* - Append it to our original
    `conc_daily` using `bind_rows()`

4.  Save the new data as `tidied_full_wq.RDS` in a folder called 'data'.
...
discharge. A function in the {dataRetrieval} package that can allow you
to easily download discharge data is `readNWISdv()`. Use this function
to download daily discharge data for all nine of the sites we are
interested in. Store the data as an R object called `q_data`, then save
it as an RDS object: `data/Q.RDS`. The site numbers are the same as
above but you need to remove `USGS-` from each site. Discharge is
`parameterCd = 00060` and you can use `renameNWISColumns()` to
