...
print(dolores_mk)
#p-value = 0.1789, not significant
```
...
dolores_slope
#Sen's slope = 0.0265165, p-value same = 0.1789, not significant
```
...
```{r}
Q = readRDS("data/Q.RDS")
```

```{r}
Q_fix <- rename(Q, siteid = site_no)
```
...
```{r}
#need to paste USGS- into Q and in Q the column for site is site_no
Q_tidied <- inner_join(Q_fix2, tidied_full_wq.RDS, by = c("date", "siteid"))
...
roaring_su <- Q_tidied %>%
  filter(basin == 'roaring', parameter == 'Sulfate') 

ggplot(roaring_su, aes(x = Flow, y = conc)) + 
  geom_point()
...
#concentration decreases with flow rate, is this called a negative relationship when it rises on the left side of the screen
...
basin_wq2 <- basin_wq %>%
  mutate(model = map(data, ~ lm(conc ~ q, data = .)))
#nope, error, "q" is not a column name and I am unsure if I use flow or ? to compare, flow also didn't work, not sure if the issue is upstream or?
...
#can't accomplish the code before this to get here.
```
...
#can't get the steps complete to get here. 
```
...
#furr::map runs faster because it runs in parrallel according to the interwebs, but I think having been able to complete the two questions above with assistance would have helped be understand differences in this lesson, in particular.
