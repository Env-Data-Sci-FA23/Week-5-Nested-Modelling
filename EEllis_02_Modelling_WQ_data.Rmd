...
Q <- Q %>%
  unite(siteid, agency_cd, site_no, sep = "-") %>% rename(date = Date)

wq_paste <- wq %>%
  mutate(siteid = ifelse(!grepl("^USGS-", siteid), paste0("USGS-", siteid), siteid))

joined_data <- inner_join(wq_paste, Q, by = c("siteid", "date"))
...
```{r}
site_plot1 <- joined_data %>% filter (siteid == "USGS-09085000" & parameter == "Bicarbonate")

plot1 <- ggplot(site_plot1, aes(x = Flow, y = conc)) + 
  geom_point() + 
   labs(x = "Discharge", y = "Bicarbonate concentration")
...
# Bicarbonate concentration decreases as discharge increases.
...
```{r}
nested_data <- joined_data %>% group_by(basin, parameter)%>% 
  nest()
...
mapped_data <- nested_data %>%
  mutate(linear_mod = map(data, ~lm(conc ~ Flow, data = .x)),
         tidy_results = map(linear_mod, tidy))
```
...
```{r}
#see tidy step linked to mapping step above

lin_mod_sum <- mapped_data %>% 
  unnest(tidy_results) %>%
...
```{r}
sig_results <- lin_mod_sum %>%
  filter(term == "Flow" & trend == "yes")

# Plotting the slopes of the significant relationships
ggplot(sig_results, aes(x = estimate, y = basin)) +
  geom_point(size = 3) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
  labs(x = "Slope of Relationship", y = "Basin") +
  ggtitle("Significant Relationships between Discharge and Concentration")
```
