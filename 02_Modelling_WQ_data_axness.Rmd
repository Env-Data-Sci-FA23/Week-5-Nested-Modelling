...
discharge_data <- readRDS('data/Q_axness.RDS')
water_quality_data <- readRDS('data/tidied_full_wq_axness.RDS')

# Add 'USGS-' prefix to site IDs in discharge_data and rename 'Date' to 'date'
discharge_data <- discharge_data %>%
  mutate(siteid = paste0('USGS-', site_no),  # Add 'USGS-' prefix
         date = as.Date(Date))  # Rename 'Date' to 'date' and ensure it's in Date format

# Now join data by date and siteid
joined_data <- inner_join(discharge_data, water_quality_data, by = c('date', 'siteid'))
...
#<<<< YOUR CODE HERE >>>>#
#Pick a specific site and ion,(example 'USGS-XXXXX' and 'MAgnesium')
filtered_data <- joined_data %>%
  filter(siteid == 'USGS-09095500', parameter == 'Magnesium')

ggplot(filtered_data, aes(x = Flow, y = conc)) +
  geom_point() +
  labs(x = 'Discharge', y = 'Concentration', title = 'Discharge vs Magnesium Concentration')
...
This relationship shows higher Mg concentration at lower discharges for this specific site. It also seems to have a stabilization at lower concentrations and larger discharges.
...
#<<<< YOUR CODE HERE >>>>#
grouped_nest_data <- grouped_nest_data %>%
  mutate(models = map(data, ~lm(conc ~ Flow, data = .x)))
...
#<<<< YOUR CODE HERE >>>>#
grouped_nest_data <- grouped_nest_data %>%
  mutate(model_summaries = map(models, broom::tidy))
...
#<<<< YOUR CODE HERE >>>>#
model_summaries_unnested <- grouped_nest_data %>%
  unnest(model_summaries) %>%
  filter(term == 'Flow') # Filter to look only at the slope term for discharge

ggplot(model_summaries_unnested, aes(x = basin, y = estimate, color = p.value < 0.05)) +
  geom_point() +
  labs(x = 'Basin', y = 'Slope of Discharge', title = 'Linear Model Summaries')
...
**`furrr`** is an extension of **`purrr`** that allows for parallel processing. It's advantageous when working with large datasets or computationally intensive tasks. The syntax is similar, but **`furrr`** can significantly speed up operations by leveraging multiple cores on your computer.

Example with this data set:

```{r}
# Unnest the model summaries for detailed analysis
detailed_analysis <- grouped_nest_data %>%
  unnest(model_summaries)

# Filter for the discharge coefficient
detailed_analysis <- detailed_analysis %>%
  filter(term == 'Flow')

# Examine the detailed statistics for each basin and parameter
detailed_analysis %>%
  arrange(basin, parameter) %>%
  select(basin, parameter, estimate, std.error, statistic, p.value)
