...
```{r}
object_1 <- tibble(siteid = c("x"),
                   basin = c("y"))

exercise_1 <- function(object_1){
  conc_wide <- readWQPdata(siteid = object_1$siteid, # pulling our siteids column in from the colorado object, becomes a vector
                         sampleMedia = "Water", # WQP also has bilogical data and sediment data
                         startDateLo = "1980-10-01", # must be formatted as YYYY-MM-DD
                         startDateHi = "2023-11-01", # must be formatted as YYYY-MM-DD
                         characteristicName = parameters) # our vector of `characteristcName`s
...
exercise_2 <- exercise_1(additional_data)
```
...
```{r}
tidied_full_wq <- rbind(conc_daily, exercise_2)
...
```{r}
#for final project - creating an annual concentration table
annual_full_wq <- tidied_full_wq %>%
  mutate(year = ifelse(month(date) >= 10, year(date) + 1, year(date))) %>%
  filter(year >= 2000) %>%
  group_by(year, parameter, basin, siteid) %>%
  summarize(total_conc = sum(conc))

final_wq <- annual_full_wq %>%
  left_join(site_sf, annual_full_wq, by = c("siteid"), relationship = "many-to-many") %>%
  select(-c(name, area, area.units, elev, elev_units, basin.x, geometry))

saveRDS(final_wq, file = 'data/final_wq.RDS')
```
...
final_wq_map <- annual_full_wq %>%
  left_join(site_sf, annual_full_wq, by = c("siteid"), relationship = "many-to-many") %>%
  select(-c(name, area, area.units, elev, elev_units, basin.x))

saveRDS(final_wq_map, file = 'data/final_wq_map.RDS')
```
...
```{r}
#trying to test out reactive() function from app
...
input_values <- list(
  basin = c("dolores", "colorado1"),
  parameter = c("calcium", "magnesium"),
  year = c("2000", "2001", "2002"),
  total_conc = c(3,14000)
)

# Execute the reactive expression with the simulated input
test_output <- isolate({
  withShinyDir({
    withMockValues({
      walk(input_values, function(val, key) {
        shinyjs::setInputValue(id = key, value = val)
      })
      result <- annual_react()
    })
  })
...
annual_react <- reactive(input_values)
```

```{r}
#server ui - copied from app.R
# Define server logic required to draw a histogram
server <- function(input, output){
  # Make a reactive object for the final_wq data by calling inputIDs to extract the values the user chose
  annual_react <- reactive(
    final_wq%>%
      filter(basin.y %in% input$basin) %>%
      filter(parameter %in% input$parameter) %>%
...
```{r}
#attempt one from chat gbt
generate_map <- function(basin, parameter, year, total_conc) {
  # Your code to filter the dataset and create the map using tmap
  # Example: Assuming 'your_data' is your dataset
  # Replace this example code with your actual logic
  filtered_data <- final_wq %>%
    filter(basin %in% input$basin.y, parameter %in% input$parameter, year %in% input$year, total_conc >= input$total_conc[1] & total_conc <= input$total_conc[2])

  tm_shape(filtered_data) +
    tm_bubbles("longitude", "latitude", size = "total_conc", col = "red") +
    tm_layout(legend.position = c("left", "bottom"))
}

# Function to generate the bar plot
generate_bar_plot <- function(basin, parameter, year, total_conc) {
  # Your code to filter the dataset and create the bar plot
  # Example: Assuming 'your_data' is your dataset
  # Replace this example code with your actual logic
  filtered_data <- final_wq %>%
    filter(basin %in% input$basin.y, parameter %in% input$parameter, year %in% input$year, total_conc >= input$total_conc[1] & total_conc <= input$total_conc[2])

  ggplot(filtered_data, aes(x = parameter, y = total_conc)) +
    geom_bar(stat = "identity") +
    labs(title = "Bar Plot of Total Concentration by Parameter", x = "Parameter", y = "Total Concentration")
}
```

```{r}
#attempt 2 from chat gbt
#generate_map <- function(basin, parameter, year, total_conc) {
  # Your code to filter the dataset and create the map using tmap
  # Example: Assuming 'your_data' is your dataset
  # Replace this example code with your actual logic
  filtered_data <- final_wq %>%
    filter(basin %in% basin.y, parameter %in% parameter, year %in% year, total_conc >= input$total_conc[1] & total_conc <= input$total_conc[2])
...
annual_react <- reactive({
  ...
  print(input$year)
  print(input$total_conc)

  final_wq %>%
    filter(basin.y %in% input$basin) %>%
    filter(parameter %in% input$parameter) %>%
    filter(year %in% input$year) %>%
    filter(total_conc >= input$total_conc[1] & total_conc <= input$total_conc[2]) %>%
    st_as_sf() %>%
    st_set_geometry("geometry")
})
